...
def open_win():
    ...
    apt = Tk()
    apt.title("Interface")
    Label(apt, text="SOMAY PHARMACY").grid(row=0, column=0, columnspan=3)
...
def again():
    ...
    root.title('SOMAYs PHARMACY')
    Label(root, text='SOMAY PHARMACY').grid(row=0, column=0, columnspan=5)
    Label(root, text="MODINAGAR, GHAZIABAD").grid(row=1, column=0, columnspan=5)
...
def open_cus():
    ...
    apt = Tk()
    apt.title("Interface")
    Label(apt, text="*** SOMAY PHARMACY ***", bg='blue', fg='white').grid(row=0, column=0)
